...
build_dir="$repo_dir/target/fp32_fma_lane_test"
mkdir -p "$build_dir"

verilator --cc --exe --build --Wno-fatal --Wno-BLKANDNBLK \
  --Mdir "$build_dir" --top-module fp32_fma_lane_wrapper \
  -I"$cvfpu_dir/src" \
  ...
  "$repo_dir/src/test/resources/fpu/fp32_fma_lane_test.cpp"

"$build_dir/Vfp32_fma_lane_wrapper"
