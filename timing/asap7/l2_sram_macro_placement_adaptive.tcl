# Adaptive centered grid placement for the SharedL2Slice SRAM macros, derived
# from compute_unit_sram_macro_placement.tcl. The fixed 6 x 3
# l2_sram_macro_placement.tcl grid lands a macro pin column under the power
# mesh and produces ~470 macro pin-access DRCs (M3/V3); this centered grid with
# a wider routing channel drops that to ~1. The channel is a tunable.
set db [ord::get_db]
...
if {[llength $macro_names] == 0} {
  error "SharedL2Slice placement expects SRAM macros, found none"
}
