...
invert/LZD-mask/mask-valid on `fire_reg0c`). The DMA byte delta is registered
before the 64-bit accumulator. Integrated synthesis STA with that RTL reaches
**755.96 MHz** (−322.83 ps): the prior FMA `csaSumReg` → `lshift_mask_valid`
limiter is gone; the core-clock path now binds on
`dmaBytesCompleted[7]` → `[63]` (64-bit counter add). Secondary paths include
L2 performance-counter increments. Hold paths still name
`l2.slices_*.fillWriteData` into the data SRAM write port.
...
1 GHz is an objective, not a milestone. Next RTL lever: pipeline or narrow
the DMA / L2 performance-counter 64-bit increments that now bind the
integrated top at ~756 MHz. Derive real parent IO budgets in parallel.
