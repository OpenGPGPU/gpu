...
class FpuRegisterFile(config: GpuConfig = GpuConfig()) extends Module {
  ...
  private def stored(index: UInt): UInt = if (config.warps == 1) {
    registers(0)(index)
  } else {
    Mux(readWarpValid, registers(io.read.warpId)(index), 0.U)
  }
  private def readWithBypass(index: UInt): UInt = Mux(
    writeEnabled && io.write.bits.warpId === io.read.warpId &&
      io.write.bits.rd === index,
    io.write.bits.data,
    stored(index)
  )

  io.rs1Data := readWithBypass(io.read.rs1)
  io.rs2Data := readWithBypass(io.read.rs2)
  io.rs3Data := readWithBypass(io.read.rs3)
