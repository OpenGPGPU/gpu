...
class ShaderFragStage(
  ...
  io.out.bits.e0 := fragE(0)
  io.out.bits.e1 := fragE(1)
  io.out.bits.e2 := fragE(2)
  io.out.bits.covered := fragCovered
  ...
  switch(state) {
    is(idle) {
      when(io.fragIn.fire) {
    fragE(0) := io.fragIn.bits.e0
    fragE(1) := io.fragIn.bits.e1
    fragE(2) := io.fragIn.bits.e2
