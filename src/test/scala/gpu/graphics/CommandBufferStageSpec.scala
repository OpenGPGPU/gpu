...
class CommandBufferStageSpec extends AnyFlatSpec {
  ...
  // Encode a SceneTriangle into a 24-word record.
  ...
  it should "decode draw-call records from a command buffer" in {
    ...
    val base = 0x4000
    val words0 = encode(record0)
    val words1 = encode(record1)
    ...
    simulate(new CommandBufferStage(GraphicsConfig())) { dut =>
      ...
      val decoded = collection.mutable.Buffer.empty[(Seq[Int], Seq[(Int, Int, Int)], Seq[Int])]
