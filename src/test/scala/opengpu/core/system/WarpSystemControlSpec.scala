...
class WarpSystemControlSpec extends AnyFlatSpec {
  ...
  it should "write only the shader vxrm immediate CSR after resume is accepted" in {
    val config = GpuConfig(lanes = 4, warps = 2)
    simulate(new WarpSystemControl(config)) { dut =>
      dut.reset.poke(true.B); dut.clock.step(); dut.reset.poke(false.B)
      dut.io.in.valid.poke(true.B)
      dut.io.in.bits.poke(0.U.asTypeOf(dut.io.in.bits))
      dut.io.in.bits.decode.decoded.csr.poke(true.B)
      dut.io.in.bits.decode.instruction.poke(0x00a15073.U) // mode 2
      dut.io.in.bits.decode.warpId.poke(1.U)
      dut.io.in.bits.decode.pc.poke(0x200.U)
      dut.io.resume.ready.poke(false.B)
      dut.io.unsupported.ready.poke(false.B)
      dut.io.resume.valid.expect(true.B)
      dut.io.vxrmWrite.valid.expect(false.B)
      dut.io.unsupported.valid.expect(false.B)
      dut.io.resume.ready.poke(true.B)
      dut.io.vxrmWrite.valid.expect(true.B)
      dut.io.vxrmWrite.bits.warpId.expect(1.U)
      dut.io.vxrmWrite.bits.data.expect(2.U)
      dut.io.resume.bits.pc.expect(0x204.U)
      dut.clock.step()
      dut.io.in.bits.decode.instruction.poke(0x00a25073.U) // mode 4
      dut.io.resume.valid.expect(false.B)
      dut.io.unsupported.valid.expect(true.B)
    }
  }

  it should "send a barrier arrival without prematurely resuming its warp" in {
    val config = GpuConfig(lanes = 4, warps = 2)
    simulate(new WarpSystemControl(config)) { dut =>
      dut.reset.poke(true.B); dut.clock.step(); dut.reset.poke(false.B)
      dut.io.in.valid.poke(true.B)
      ...
      dut.io.in.bits.decode.pc.poke(0x200.U)
      dut.io.in.bits.decode.warpId.poke(1.U)
      dut.io.in.bits.decode.activeMask.poke(0x7.U)
      dut.io.in.bits.decode.decoded.barrier.poke(true.B)
      dut.io.restore.ready.poke(false.B)
      dut.io.resume.ready.poke(true.B)
      dut.io.barrier.ready.poke(false.B)
      dut.io.unsupported.ready.poke(false.B)

      dut.io.barrier.valid.expect(true.B)
      dut.io.barrier.bits.pc.expect(0x204.U)
      dut.io.resume.valid.expect(false.B)
      dut.io.in.ready.expect(false.B)
      dut.io.barrier.ready.poke(true.B)
      dut.io.in.ready.expect(true.B)
    }
  }
